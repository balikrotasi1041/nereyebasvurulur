...
.source-route-link{display:inline-block;margin-top:8px;font-size:.82rem;font-weight:700;color:var(--blue)}
...
(function(){
  ...
  var prefillRecipient=(params.get("merci")||"").slice(0,160);
  var prefillSubject=(params.get("konu")||"").slice(0,180);
  var prefillType=(params.get("tur")||"").slice(0,180);
  var prefillSource=(params.get("kaynak")||"").slice(0,220);
  var sourceRouteLink=document.getElementById("sourceRouteLink");
  if(prefillRecipient) fields.recipient.value=prefillRecipient;
  if(prefillSubject){fields.subject.value=prefillSubject;subjectEdited=true}
  if(/^\/konu\/[a-z0-9-]+\/$/.test(prefillSource)){sourceRouteLink.href=prefillSource;sourceRouteLink.hidden=false}
  if(prefillRecipient||prefillSubject){
    status.textContent=(prefillType?prefillType+" için ":"")+"konu ve merci rotadan getirildi; olayları, tarihleri, talebinizi ve ekleri siz tamamlayın.";
    status.style.color="var(--ok)";
  }
