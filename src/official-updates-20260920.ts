...
export function refreshSeptember20(item: Announcement): Announcement {
  if (item.slug === "2026-gsb-yurt-basvuru-sonuclari-sorgulama") return {
    ...
    // A personal dormitory result is not a university registration procedure.
    relatedPathKeys: [],
  ...
  if (item.slug === "2027-cks-basvurulari-basladi") {
    const { deadlineAt: _previousInferredHour, ...datedItem } = item;
    return {
      ...datedItem, lastModified: "2026-09-20",
      deadlineDate: "2026-12-31",
      relatedPathKeys: ["Tarım, Hayvancılık, Orman ve Kırsal > Çiftçi Kayıt Sistemi > ÇKS kaydı"]
    };
  }
  return item;
}
